...
CREATE VIEW plotar2 AS 
    SELECT fluxoPorSensor from (SELECT estacao.nome, nomeLocal, ifnull(sum(contagem), 0) as fluxoPorSensor from dadoSensor
	right join sensor on fkSensor = idSensor
    join estacao on fkEstacao = idEstacao
    group by idSensor ORDER BY idEstacao DESC, nomeLocal) as subQuerry123;
SELECT * FROM plotar2;
...
SELECT * FROM (select sum(contagem)as cont, fkEstacao, nome from dadoSensor
join sensor ON fkSensor = idSensor
join estacao ON fkEstacao = idEstacao
where date_format(horario, '%d/%m/%y') = date_format(current_date(), '%d/%m/%y')
group by idEstacao ORDER BY idEstacao DESC) as subQuerryDia;    



CREATE VIEW dadoDashDia AS SELECT cont FROM (select ifnull(sum(contagem), 0)as cont,  fkEstacao, nome from dadoSensor
join sensor ON fkSensor = idSensor
join estacao ON fkEstacao = idEstacao
where date_format(horario, '%d/%m/%y') = date_format(now(), '%d/%m/%y')
group by idEstacao
ORDER BY idEstacao DESC) as subQuerryDia;
DROP VIEW dadoDashDia;
select * from dadoDashDia;

select ifnull(sum(contagem), 0)as cont,  fkEstacao, nome from dadoSensor
join sensor ON fkSensor = idSensor
join estacao ON fkEstacao = idEstacao
where date_format(horario, '%d/%m/%y') = date_format(now(), '%d/%m/%y')
group by idEstacao
ORDER BY idEstacao DESC;
...
INSERT INTO dadoSensor VALUES 
	...
	(default, 43, 1, default),
	(default, 49, 1, default),
	(default, 56, 1, default),
	(default, 63, 1, default);

SELECT cont FROM (select sum(contagem) as cont, fkSensor, fkEstacao, nome from dadoSensor
join sensor ON fkSensor = idSensor
join estacao ON fkEstacao = idEstacao
where horario between DATE_SUB(CURDATE(), INTERVAL 7 day) AND NOW()
group by fkSensor) as subQuerrySemana;

CREATE VIEW dadoDashSemana AS SELECT cont FROM (select sum(contagem) as cont, fkEstacao, nome from dadoSensor
join sensor ON fkSensor = idSensor
join estacao ON fkEstacao = idEstacao
where horario between DATE_SUB(CURDATE(), INTERVAL 7 day) AND NOW()
group by idEstacao ORDER BY idEstacao DESC) as subQuerrySemana;
...
CREATE VIEW dadoDashMes AS SELECT cont FROM (select sum(contagem) as cont, fkEstacao, nome from dadoSensor
join sensor ON fkSensor = idSensor
join estacao ON fkEstacao = idEstacao
where horario between DATE_SUB(CURDATE(), INTERVAL 30 day) AND NOW()
group by idEstacao order by idEstacao DESC) as subQuerryMes;
